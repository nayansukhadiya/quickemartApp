      ...
      {brand.length === 0 ? (
        ...
        <Link to={`/search?q=${item}`} className='cardBrand'>
            ...
            title={item}
              ...
              src={require(`../../assets/images/Brand_Logo/${item.toLowerCase().replace(/ /g, "_")}.png`)}
              alt={item}
              onError={(e) => {
                e.target.onerror = null; // Prevents infinite loop
                e.target.src = '../../assets/images/Brand_Logo/default.png'; // Fallback image
              }}
            />
            <h6>{item}</h6>
        </Link>
